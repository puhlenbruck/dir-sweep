...
validatedFilterOptions thresholdTime maxKeep minKeep | maxLessThanMin = Left "MaxKeep cannot be less than MinKeep"
...
filterFiles FilterOptions{thresholdTime, maxKeep, minKeep} files = 
  ...
  where minKeepValue = fromMaybe 0 minKeep
        go count (x:xs) | maybe False (count >=) maxKeep || 
                          maybe False (modifyTime x <) thresholdTime = reverse (x:xs)
